...
// ─── Multi-channel spill buffer for shim_reqresp_once_mc ───
// Lock-free SPSC ring: reqresp_once_mc enqueues non-matching packets,
// shim_rx_burst / dispatch_poll dequeues before NIC burst.
...
static void shim_dbg_init_path(HINSTANCE hDll) {
    ...
    if (len == 0) {
        ...
        strcat_s(g_dbg_path, MAX_PATH, "\\eal_debug.txt");
        return;
    }
    // Replace DLL filename with eal_debug.txt
...
// Write to debug file (in the same directory as dpdk_shim.dll / exe)
static void shim_dbg(const char *fmt, ...) {
    ...
    const char *path = (g_dbg_path[0] != '\0') ? g_dbg_path : "eal_debug.txt";
...
static LONG shim_seh_filter(EXCEPTION_POINTERS *ep) {
    ...
    shim_dbg("[SHIM] SEH EXCEPTION! code=0x%08lX\n", code);
    shim_dbg("[SHIM]   instruction at  : %p\n", instr_addr);
    shim_dbg("[SHIM]   fault address   : %p (%s)\n", fault_addr,
    ...
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
        ...
        shim_dbg("[SHIM]   crash module   : %s + 0x%llX\n", modName, (unsigned long long)offset);
    }
...
static void clean_dpdk_runtime(void) {
    ...
    if (cleaned > 0)
        shim_dbg("[SHIM] cleaned %d stale runtime files from %s\n", cleaned, runtime_dir);
}

// EAL init wrapper — enables SeLockMemory, sets working set, then inits EAL
DPDK_API int shim_eal_init(int argc, char **argv) {
    ...
    if (g_eal_initialized) {
        shim_dbg("[SHIM] WARNING: shim_eal_init called again (already initialized). Returning -98.\n");
        return -98;
    }

    /* Log received args for debugging */
    shim_dbg("[SHIM] shim_eal_init called with argc=%d\n", argc);
    for (int i = 0; i < argc; i++)
        shim_dbg("[SHIM]   argv[%d] = \"%s\"\n", i, argv[i] ? argv[i] : "(null)");
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
        ...
        if (LookupPrivilegeValueA(NULL, "SeLockMemoryPrivilege", &tp.Privileges[0].Luid)) {
            ...
            shim_dbg("[SHIM] AdjustTokenPrivileges(SeLockMemory) ok=%d err=%lu\n", adj_ok, adj_err);
        } else {
            shim_dbg("[SHIM] LookupPrivilegeValue FAIL err=%lu\n", GetLastError());
        }
        CloseHandle(hToken);
    } else {
        shim_dbg("[SHIM] OpenProcessToken FAIL err=%lu\n", GetLastError());
    }

    /* Check current working set limits */
    SIZE_T cur_min = 0, cur_max = 0;
    GetProcessWorkingSetSize(GetCurrentProcess(), &cur_min, &cur_max);
    shim_dbg("[SHIM] current WS min=%zu max=%zu\n", cur_min, cur_max);
    ...
    shim_dbg("[SHIM] SetProcessWorkingSetSizeEx(%zu,%zu) = %d err=%lu\n",
        ws_min, ws_max, ws_ok, ws_ok ? 0 : GetLastError());

    /* Pass C# args directly to EAL (SEH protected with detailed crash info) */
    int ret;
    shim_dbg("[SHIM] calling rte_eal_init...\n");
    __try {
        ret = rte_eal_init(argc, argv);
    } __except(shim_seh_filter(GetExceptionInformation())) {
        g_eal_initialized = 1;
        return -99;
    }
    shim_dbg("[SHIM] rte_eal_init returned %d\n", ret);
...
DPDK_API uint16_t shim_rx_burst(uint16_t port_id, uint16_t queue_id, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
    uint16_t total = 0;
...
// 2. TX Burst Wrapper
DPDK_API uint16_t shim_tx_burst(uint16_t port_id, uint16_t queue_id, struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
    return rte_eth_tx_burst(port_id, queue_id, tx_pkts, nb_pkts);
}

// 3. Mbuf Data Pointer Wrapper (rte_pktmbuf_mtod)
// C# needs a raw pointer to the packet data
DPDK_API void* shim_pktmbuf_mtod(struct rte_mbuf *m) {
    return rte_pktmbuf_mtod(m, void*);
}

// 4. Mbuf Data Length Wrapper
DPDK_API uint16_t shim_pktmbuf_data_len(struct rte_mbuf *m) {
    return rte_pktmbuf_data_len(m);
}

// 5. Mbuf Free Wrapper
DPDK_API void shim_pktmbuf_free(struct rte_mbuf *m) {
    rte_pktmbuf_free(m);
}

// 6. Mbuf Allocation Wrapper
DPDK_API struct rte_mbuf* shim_pktmbuf_alloc(struct rte_mempool *mp) {
    return rte_pktmbuf_alloc(mp);
}

// 7. Mbuf Append Wrapper (for TX packet construction)
DPDK_API char* shim_pktmbuf_append(struct rte_mbuf *m, uint16_t len) {
    return rte_pktmbuf_append(m, len);
}

// 8. Mbuf Packet Length Wrapper
DPDK_API uint32_t shim_pktmbuf_pkt_len(struct rte_mbuf *m) {
...
DPDK_API const char* shim_version(void) {
    __try {
        return rte_version();
    } __except(1) {
        shim_dbg("[SHIM] rte_version SEH EXCEPTION\n");
        return NULL;
    }
}
...
DPDK_API int shim_eth_dev_count_avail(void) {
    __try {
        ...
        shim_dbg("[SHIM] rte_eth_dev_count_avail SEH EXCEPTION\n");
        return -1;
    }
}

// 9c. MAC address get (SEH protected)
DPDK_API int shim_eth_macaddr_get(uint16_t port_id, struct rte_ether_addr *mac_addr) {
    __try {
        return rte_eth_macaddr_get(port_id, mac_addr);
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_macaddr_get SEH EXCEPTION port=%u\n", port_id);
        return -1;
    }
}

// 9d. Link status get (SEH protected)
DPDK_API int shim_eth_link_get_nowait(uint16_t port_id, struct rte_eth_link *link) {
    __try {
        return rte_eth_link_get_nowait(port_id, link);
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_link_get_nowait SEH EXCEPTION port=%u\n", port_id);
...
DPDK_API struct rte_mempool* shim_pktmbuf_pool_create_safe(
    ...
    __try {
        ...
        shim_dbg("[SHIM] rte_pktmbuf_pool_create SEH EXCEPTION\n");
        return NULL;
    }
}

// 9f. Device stop (SEH protected)
DPDK_API int shim_eth_dev_stop(uint16_t port_id) {
    __try {
        return rte_eth_dev_stop(port_id);
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_dev_stop SEH EXCEPTION port=%u\n", port_id);
        return -1;
    }
}

// 9g. Device close (SEH protected)
DPDK_API int shim_eth_dev_close(uint16_t port_id) {
    __try {
        return rte_eth_dev_close(port_id);
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_dev_close SEH EXCEPTION port=%u\n", port_id);
        return -1;
    }
}

// 9h. EAL cleanup (SEH protected)
DPDK_API int shim_eal_cleanup(void) {
    ...
    __try {
        ...
        shim_dbg("[SHIM] rte_eal_cleanup SEH EXCEPTION\n");
        return -1;
    }
}

// 9i. Eth stats get (SEH protected)
DPDK_API int shim_eth_stats_get(uint16_t port_id, struct rte_eth_stats *stats) {
    __try {
        return rte_eth_stats_get(port_id, stats);
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_stats_get SEH EXCEPTION port=%u\n", port_id);
...
DPDK_API int shim_eth_stats_reset(uint16_t port_id) {
    __try {
        rte_eth_stats_reset(port_id);
        return 0;
    } __except(1) {
        shim_dbg("[SHIM] rte_eth_stats_reset SEH EXCEPTION port=%u\n", port_id);
        return -1;
    }
}
...
DPDK_API int shim_simple_port_setup(uint16_t port_id, struct rte_mempool *mbuf_pool,
    ...
    fprintf(stderr, "SHIM: link_speeds=0x%08X (%s)\n", port_conf.link_speeds,
            port_conf.link_speeds == 0 ? "autoneg" : "fixed");
    fprintf(stderr, "SHIM: port_setup start (port=%u, pool=%p)\n", port_id, (void*)mbuf_pool);
    fflush(stderr);

    if (!rte_eth_dev_is_valid_port(port_id)) {
        fprintf(stderr, "SHIM: invalid port %u\n", port_id);
        return -1;
    }
    fprintf(stderr, "SHIM: [1/7] valid port OK\n"); fflush(stderr);

    memset(&dev_info, 0, sizeof(dev_info));
    retval = rte_eth_dev_info_get(port_id, &dev_info);
    if (retval != 0) { fprintf(stderr, "SHIM: [2/7] dev_info_get FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [2/7] dev_info OK (max_rx_q=%u, max_tx_q=%u)\n",
    ...
    if (retval != 0) { fprintf(stderr, "SHIM: [3/7] dev_configure FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [3/7] dev_configure OK\n"); fflush(stderr);

    retval = rte_eth_dev_adjust_nb_rx_tx_desc(port_id, &nb_rxd, &nb_txd);
    if (retval != 0) { fprintf(stderr, "SHIM: [4/7] adjust_desc FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [4/7] adjust_desc OK (rxd=%u, txd=%u)\n", nb_rxd, nb_txd); fflush(stderr);

    // Windows에서 NUMA 미감지 시 socket_id가 -1이 되므로 0으로 보정
    int socket_id = rte_eth_dev_socket_id(port_id);
    if (socket_id < 0) socket_id = 0;
    fprintf(stderr, "SHIM: socket_id=%d\n", socket_id); fflush(stderr);

    // RX queue setup
    retval = rte_eth_rx_queue_setup(port_id, 0, nb_rxd, socket_id, NULL, mbuf_pool);
    if (retval < 0) { fprintf(stderr, "SHIM: [5/7] rx_queue_setup FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [5/7] rx_queue_setup OK\n"); fflush(stderr);

    // TX queue setup
    txconf = dev_info.default_txconf;
    txconf.offloads = port_conf.txmode.offloads;
    retval = rte_eth_tx_queue_setup(port_id, 0, nb_txd, socket_id, &txconf);
    if (retval < 0) { fprintf(stderr, "SHIM: [6/7] tx_queue_setup FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [6/7] tx_queue_setup OK\n"); fflush(stderr);

    // Start the Ethernet port (Windows SEH로 크래시 보호)
    fprintf(stderr, "SHIM: [7/7] dev_start calling...\n"); fflush(stderr);
    __try {
        retval = rte_eth_dev_start(port_id);
    } __except(1) {
        fprintf(stderr, "SHIM: [7/7] dev_start SEH EXCEPTION caught!\n");
        fflush(stderr);
        return -99;
    }
    if (retval < 0) { fprintf(stderr, "SHIM: [7/7] dev_start FAIL=%d\n", retval); return retval; }
    fprintf(stderr, "SHIM: [7/7] dev_start OK\n"); fflush(stderr);
    ...
    if (!g_spill_ring) {
        ...
        if (g_spill_ring)
            fprintf(stderr, "SHIM: spill_ring created (256 entries, SPSC)\n");
        else
            fprintf(stderr, "SHIM: WARNING — spill_ring creation failed!\n");
    }

    fprintf(stderr, "SHIM: port_setup DONE\n"); fflush(stderr);
...
DPDK_API int shim_reqresp_once(
    uint16_t port_id,
...
DPDK_API int shim_reqresp_once_mc(
    uint16_t port_id,
    struct rte_mempool *pool,
    const uint8_t *template_pkt,
    uint16_t pkt_len,
...
DPDK_API int shim_reqresp_batch(
    uint16_t port_id,
    struct rte_mempool *pool,
    const uint8_t *template_pkt,
    uint16_t pkt_len,
...
static int ftp_poll_wait_session(ftp_client_t *ftp, int timeout_ms)
{
    ...
    while (!ftp->op_done) {
        if ((int)((DWORD)GetTickCount64() - start) >= timeout_ms) {
            shim_dbg("[SHIM] ftp_poll_wait: TIMEOUT after %dms (state=%d)\n",
        ...
        if (ftp->state != last_state) {
            shim_dbg("[SHIM] ftp_poll_wait: state %d -> %d\n", last_state, ftp->state);
            last_state = ftp->state;
        }
    }
...
DPDK_API int shim_lwip_init(uint16_t port_id, void *pool,
...
DPDK_API void shim_lwip_stop(void)
{
    dpdk_lwip_stop();
}

DPDK_API int shim_lwip_poll(int max_ms)
{
    return dpdk_lwip_poll_ms(max_ms);
}

/* --- lwIP reference-counted init/stop for multi-session FTP --- */
DPDK_API int shim_lwip_init_ref(uint16_t port_id, void *pool,
    uint32_t ip, uint32_t mask, uint32_t gw, const uint8_t *mac)
{
    ...
    g_lwip_ref_count++;
    shim_dbg("[SHIM] lwip_init_ref: ref_count=%d\n", g_lwip_ref_count);
    return 0;
}

DPDK_API void shim_lwip_stop_ref(void)
{
    if (g_lwip_ref_count <= 0) return;
    g_lwip_ref_count--;
    shim_dbg("[SHIM] lwip_stop_ref: ref_count=%d\n", g_lwip_ref_count);
    if (g_lwip_ref_count == 0) {
        dpdk_lwip_stop();
    }
}
...
DPDK_API int shim_ftp_connect_sync(const char *server_ip, uint16_t port,
    const char *user, const char *pass, int timeout_ms)
{
    ...
    if (ip == 0) {
        shim_dbg("[SHIM] ftp_connect_sync: invalid IP '%s'\n", server_ip);
        return -1;
    }

    shim_dbg("[SHIM] ftp_connect_sync: connecting to %s:%u\n", server_ip, port);
    ...
    if (ftp_poll_wait(timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_connect_sync: connect/welcome failed (state=%d)\n", g_ftp.state);
    ...
    /* Phase 2: USER */
    {
        ...
        if (ftp_poll_wait(timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_sync: USER failed\n");
            ftp_disconnect(&g_ftp);
            return -1;
        }
    }
    ...
    if (g_ftp.response_code == 230) {
        shim_dbg("[SHIM] ftp_connect_sync: logged in (no password), setting binary mode\n");
    ...
    /* Phase 3: PASS */
    {
        ...
        if (ftp_poll_wait(timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_sync: PASS failed\n");
            ftp_disconnect(&g_ftp);
            return -1;
        }
    }
...
set_binary_mode:
     ...
     * corrupting binary file transfers (.bin, .dat, etc.) */
    {
        ...
        if (ftp_send_cmd(&g_ftp, "TYPE I\r\n") != 0) {
            shim_dbg("[SHIM] ftp_connect_sync: TYPE I send failed\n");
            ftp_disconnect(&g_ftp);
            return -1;
        }
        if (ftp_poll_wait(timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_sync: TYPE I timeout\n");
            ftp_disconnect(&g_ftp);
            return -1;
        }
        if (g_ftp.response_code != 200) {
            shim_dbg("[SHIM] ftp_connect_sync: TYPE I unexpected response %d\n",
                     g_ftp.response_code);
            ftp_disconnect(&g_ftp);
            return -1;
        }
    ...
    shim_dbg("[SHIM] ftp_connect_sync: logged in OK (binary mode)\n");
    return 0;
}

DPDK_API int shim_ftp_disconnect(void)
{
...
DPDK_API int shim_ftp_pwd(char *buf, int buf_size)
{
...
DPDK_API int shim_ftp_cwd(const char *path)
{
    if (g_ftp.state != FTP_STATE_READY) return -1;
...
DPDK_API int shim_ftp_list_sync(char *buf, int buf_size, int *out_len, int timeout_ms)
{
    shim_dbg("[SHIM] ftp_list_sync: state=%d, timeout=%d\n", g_ftp.state, timeout_ms);

    if (g_ftp.state != FTP_STATE_READY) {
        shim_dbg("[SHIM] ftp_list_sync: NOT READY (state=%d)\n", g_ftp.state);
        return -1;
    }

    if (ftp_start_list(&g_ftp, (uint8_t *)buf, (uint32_t)buf_size) != 0) {
        shim_dbg("[SHIM] ftp_list_sync: ftp_start_list failed\n");
        return -1;
    }

    shim_dbg("[SHIM] ftp_list_sync: PASV sent, polling...\n");
    int ret = ftp_poll_wait(timeout_ms);
    shim_dbg("[SHIM] ftp_list_sync: poll_wait returned %d, state=%d, recv_len=%u\n",
             ret, g_ftp.state, g_ftp.recv_len);

    if (ret != 0) {
        shim_dbg("[SHIM] ftp_list_sync: FAILED (state=%d, op_result=%d, resp_code=%d)\n",
                 g_ftp.state, g_ftp.op_result, g_ftp.response_code);
        shim_dbg("[SHIM] ftp_list_sync: response_buf='%.*s'\n",
...
DPDK_API int shim_ftp_download_sync(const char *path, uint8_t *buf, int buf_size,
    int *out_len, int timeout_ms)
{
    if (g_ftp.state != FTP_STATE_READY) {
        shim_dbg("[SHIM] ftp_download_sync: NOT READY (state=%d)\n", g_ftp.state);
        return -1;
    }
    ...
    if (ftp_poll_wait(timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_download_sync: timeout/error (state=%d, resp=%d)\n",
...
DPDK_API int shim_ftp_upload_sync(const char *path, const uint8_t *data, int data_len,
    int timeout_ms)
{
    if (g_ftp.state != FTP_STATE_READY) return -1;
    ...
    if (ftp_poll_wait(timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_upload_sync: timeout/error (state=%d, resp=%d)\n",
                 g_ftp.state, g_ftp.response_code);
        ftp_cleanup_data(&g_ftp);
        return -1;
    }
...
DPDK_API int shim_ftp_get_state(void)
{
    return g_ftp.state;
}
...
DPDK_API int shim_ftp_connect_sync_ex(int session_id,
    ...
    const char *user, const char *pass, int timeout_ms)
{
    ...
    uint32_t ip = parse_ip_str(server_ip);
    if (ip == 0) {
        shim_dbg("[SHIM] ftp_connect_ex[%d]: invalid IP '%s'\n", session_id, server_ip);
        return -1;
    }

    shim_dbg("[SHIM] ftp_connect_ex[%d]: connecting to %s:%u\n", session_id, server_ip, port);

    if (ftp_connect(ftp, ip, port) != 0)
        return -1;

    if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_connect_ex[%d]: connect/welcome failed (state=%d)\n", session_id, ftp->state);
        ftp_disconnect(ftp);
        return -1;
    }
    ...
    /* USER */
    {
        ...
        if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_ex[%d]: USER failed\n", session_id);
            ftp_disconnect(ftp);
            return -1;
        }
    }
    ...
    /* PASS */
    {
        ...
        if (ftp_send_cmd(ftp, cmd) != 0) { ftp_disconnect(ftp); return -1; }
        if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_ex[%d]: PASS failed\n", session_id);
            ftp_disconnect(ftp);
            return -1;
        }
    }
...
set_binary_ex:
    /* TYPE I (binary mode) */
    {
        ...
        if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
            shim_dbg("[SHIM] ftp_connect_ex[%d]: TYPE I timeout\n", session_id);
            ftp_disconnect(ftp);
            return -1;
        }
        if (ftp->response_code != 200) {
            shim_dbg("[SHIM] ftp_connect_ex[%d]: TYPE I unexpected %d\n", session_id, ftp->response_code);
            ftp_disconnect(ftp);
            return -1;
        }
    }

    shim_dbg("[SHIM] ftp_connect_ex[%d]: logged in OK (binary mode)\n", session_id);
    return 0;
}

DPDK_API int shim_ftp_disconnect_ex(int session_id)
{
    ftp_client_t *ftp = get_session(session_id);
    if (!ftp) return -1;
...
DPDK_API int shim_ftp_download_sync_ex(int session_id,
    ...
    int *out_len, int timeout_ms)
{
    ...
    if (ftp->state != FTP_STATE_READY) {
        shim_dbg("[SHIM] ftp_download_ex[%d]: NOT READY (state=%d)\n", session_id, ftp->state);
        return -1;
    }
    ...
    if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_download_ex[%d]: timeout/error (state=%d, resp=%d)\n",
...
DPDK_API int shim_ftp_upload_sync_ex(int session_id,
    ...
    int timeout_ms)
{
    ...
    if (ftp_poll_wait_session(ftp, timeout_ms) != 0) {
        shim_dbg("[SHIM] ftp_upload_ex[%d]: timeout/error (state=%d, resp=%d)\n",
                 session_id, ftp->state, ftp->response_code);
        ftp_cleanup_data(ftp);
        return -1;
    }
...
DPDK_API int shim_ftp_get_state_ex(int session_id)
{
    ftp_client_t *ftp = get_session(session_id);
    if (!ftp) return -1;
    return ftp->state;
...
// ================================================================
DPDK_API int shim_read_debug_log(char *buf, int buf_size) {
...
// ================================================================
DPDK_API int shim_check_hugepage(int test_mb) {
    ...
    if (p == NULL) {
        ...
        shim_dbg("[SHIM] hugepage check FAIL: %dMB err=%lu\n", test_mb, err);
        return (int)err;
    }
    VirtualFree(p, 0, MEM_RELEASE);
    shim_dbg("[SHIM] hugepage check OK: %dMB\n", test_mb);
    return 0;
}

/* DllMain — safety net for hugepage cleanup on process exit/crash.
 * If the app exits without calling shim_eal_cleanup(), this ensures
 ...
 * ============================================================ */
DPDK_API uint16_t shim_dispatch_poll(
 ...
 * ============================================================ */
DPDK_API void shim_lwip_set_external_rx(int enabled)
{
    dpdk_netif_set_external_rx(enabled);
    shim_dbg("[SHIM] lwip_set_external_rx: %d\n", enabled);
}
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
    ...
    if (fdwReason == DLL_PROCESS_ATTACH) {
        shim_dbg_init_path(hinstDLL);
    }

    if (fdwReason == DLL_PROCESS_DETACH && g_eal_initialized) {
        shim_dbg("[SHIM] DllMain DETACH — running eal_cleanup\n");
        g_eal_initialized = 0;
        __try {
            rte_eal_cleanup();
        } __except(1) {
            shim_dbg("[SHIM] DllMain cleanup SEH exception\n");
        }
    }
    return TRUE;
}
