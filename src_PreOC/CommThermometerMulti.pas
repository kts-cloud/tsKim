...
const
  ...
  COMMTHERMOMETER_MAX_CH_NUM = 7;
  COMMTHERMOMETER_MAX_CH_CNT = 3;
type
//  {$IFNDEF GUIMessage}
//    {$DEFINE GUIMessage}
    /// <summary> GUI Message for WM_COPYDATA </summary>
    TGUIThermometerMessage = packed record
      MsgType : Integer;
      Channel : Integer;
      Mode    : Integer;
      Param  : Integer;
    ...
    PGUIThermometerMessage = ^TGUIThermometerMessage;
//  {$ENDIF}
  ...
  public
    ...
    CurrentValue : array[0..COMMTHERMOMETER_MAX_CH_NUM] of Double; //읽은 값
    State: Byte; //현재 상태
  public
    ...
    procedure QueryData(nCh: Byte = 1);
    procedure TestProcessData(AData: TBytes; Count: Integer);
    function SendBuf(AData: array of Byte; ASize: Integer): Integer;
  end;

var
  g_CommThermometer: TCommThermometerMulti;
...
procedure TCommThermometerMulti.AddLog(sLog: String);
begin
  SendMessageMain(COMMTHERMOMETER_ADDLLOG, 0, 0, sLog);
end;

constructor TCommThermometerMulti.Create(hMain :THandle; nMsgType: Integer; nInterval: Integer);
begin
  m_hMain:= hMain;
  m_nMsgType:= nMsgType;
...
destructor TCommThermometerMulti.Destroy;
begin
  Disconnect;
...
function TCommThermometerMulti.Connect(nPort: Integer): boolean;
begin
  Result:= False;
...
procedure TCommThermometerMulti.Disconnect;
begin
  m_tmrCycle.Enabled:= False;
  m_VaComm.Close;
...
procedure TCommThermometerMulti.QueryData(nCh: Byte);
var
...
procedure TCommThermometerMulti.VaCommRxChar(Sender: TObject; Count: Integer);
var
  Buff: array [0..100]of Byte;
  nCh : Integer;
  wCrc: Word;
...
begin
(*
  //01 04 04 00 D3 00 01 CB BD
  //22.1
  Buff[0]:= $01; //Slave Address
  Buff[1]:= $04; //Function - Read Input Register (func04 = 04H)
  Buff[2]:= $04; //Byte Count
  Buff[3]:= $00; //Data(Hi) //현재 측정 값 211
  Buff[4]:= $D3; //Data(Lo)
  Buff[5]:= $00; //Data(Hi) //소수점 위치(0: 0, 1:0.0, 2:0.00, 3:0.000)
  Buff[6]:= $01; //Data(Lo)
  Buff[7]:= $CB; //Lo(wCrc);
  Buff[8]:= $BD; //Hi(wCrc);
*)
  ...
  if m_nBuffCount >= 9 then begin
    //Validation
    if (m_Buff[1] = $04) then begin //읽기 응답
      nCount:= m_Buff[2];
      ...
      if wRecvCrc <> wCrc then begin
        ...
        //정상 패킷
        nCh := m_Buff[0] - 1;
        if nCh in [0.. COMMTHERMOMETER_MAX_CH_NUM] then begin
          wValue:= m_Buff[3] * 256 + m_Buff[4];
          CurrentValue[nCh] := wValue * 0.1; //소숫점 자리수 처리. 기기설정에서 1로 설정(301002(03E9) 값 )
          m_nTimeoutCount:= 0;
          SendMessageMain(COMMTHERMOMETER_UPDATE, wValue, m_Buff[0], '');
        end;
      ...
      //이상 패킷
      AddLog('Packet mismatch: ' + BufferToHex(@m_Buff[0], m_nBuffCount));
    end;
    m_nBuffCount:= 0; //m_nBuffCount:= m_nBuffCount - 9;
  end;
end;

procedure TCommThermometerMulti.TestProcessData(AData: TBytes; Count: Integer);
var
  wCrc: Word;
  wRecvCrc: Word;
  wValue: Word;
...
begin
  ...
  //01 04 04 00 D3 00 01 CB BD
  //
  AddLog('Recv: ' + BufferToHex(@AData[0], Count));

  CopyMemory(@m_Buff[m_nBuffCount], @AData[0], Count);
  m_nBuffCount:= m_nBuffCount + Count;
  if m_nBuffCount >= 9 then begin
    //Validation
    //if (Buff[0] = $01) and (Buff[1] = $04) then begin
    if (m_Buff[1] = $04) then begin
      nCount:= m_Buff[2];
      wCrc:= CalcCRC16(m_Buff, nCount + 3);
      CopyMemory(@wRecvCrc, @m_Buff[nCount + 3], 2);
      if wRecvCrc <> wCrc then begin
...
procedure TCommThermometerMulti.tmrCycleTimer(Sender: TObject);
var
  nCh: Byte;
begin
  //Send Query
  if Connected then begin
    inc(m_nTimeoutCount);
    if m_nTimeoutCount = 16 then begin
      //Timeout 발생
      SendMessageMain(COMMTHERMOMETER_CONNECT, 2, 0, 'Query Timeout');
    end;
    if m_nTimeoutCount > 100 then begin
      m_nTimeoutCount:= 100;
    end;

    nCh:= (m_tmrCycle.Tag mod 8) + 1;
    QueryData(nCh);
    m_tmrCycle.Tag:= (m_tmrCycle.Tag + 1) mod 8; //0 ~ 2 순환
  end;
end;

function TCommThermometerMulti.SendBuf(AData: array of Byte; ASize: Integer): Integer;
begin
  AddLog('SendBuf: ' + BufferToHex(@AData[0], ASize));
  m_VaComm.WriteBuf(AData, ASize);
end;

procedure TCommThermometerMulti.SendMessageMain(nMode, nParam, nParam2: Integer; sMsg: String);
var
  cds         : TCopyDataStruct;
  GUIMessage     : TGUIThermometerMessage;
begin
