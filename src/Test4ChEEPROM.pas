...
uses
  ...
  HandBCR, GMesCom, pasScriptClass, AutoBCRClient, DefGmes, Vcl.Buttons,
  DefPlc, PlcTcp_EEPROM;

type
  TfrmTest4ChEEPROM = class(TForm)
    imgCheckBox: TImage;
...
//    procedure DisplaySeq;
    ...
    procedure getEepromPlcBcrData(nIndexPos:Smallint; naBcrLen: array of Integer; saBcrData: array of string);
...
//    procedure ShowAgingTime(nTime : Integer);
  public
    ...
    procedure SetEepromPlcBcrData;
//    procedure SetLanguage(nIdx : Integer);
  end;

var
  frmTest4ChEEPROM: TfrmTest4ChEEPROM;
...
procedure TfrmTest4ChEEPROM.btnErrorDisplayClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnErrorDisplayClick'); //IMSI
  pnlErrAlram.Visible := False;
end;

procedure TfrmTest4ChEEPROM.btnRepeatClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnRepeatClick'); //IMSI
//  JigLogic[Self.Tag].StartIspd_A_Repeat;
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_3);
end;

procedure TfrmTest4ChEEPROM.btnRetryClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnRetryClick'); //IMSI
...
procedure TfrmTest4ChEEPROM.BtnStartTestClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.BtnStartTestClick'); //IMSI
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_START);

end;

procedure TfrmTest4ChEEPROM.BtnStopTestClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.BtnStopTestClick'); //IMSI
  JigLogic[Self.Tag].StopIspd_A;
end;

procedure TfrmTest4ChEEPROM.btnSWCancel1Click(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnSWCancel1Click'); //IMSI
  JigLogic[Self.Tag].StopIspd_A;
end;

procedure TfrmTest4ChEEPROM.btnSWNext1Click(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnSWNext1Click'); //IMSI
//  pnlErrAlram.Visible := True;
//  Pg[0].TestFunc;
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_9);
end;

procedure TfrmTest4ChEEPROM.btnVirSerialClick(Sender: TObject);
var
  sTempBcr : string;
  sTempPID : string;
  i : integer;
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnVirSerialClick'); //IMSI
  sTempBcr := '';
  sTempPID := '';
  ...
  if Common.SystemInfo.UseAutoBCR then sTempBcr := 'HND818P200003664'
...
procedure TfrmTest4ChEEPROM.btnVirtualKeyClick(Sender: TObject);
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.btnVirtualKeyClick'); //IMSI
...
procedure TfrmTest4ChEEPROM.chkPgClick(Sender: TObject);
var
  i : integer;
  wCh : word;
  sTemp : string;
begin
  Common.MLog(Self.Tag,'TfrmTest4ChEEPROM.chkPgClick'); //IMSI
...
procedure TfrmTest4ChEEPROM.ClearChData(nCh: Integer);
var
  i: Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.ClearChData'); //IMSI
...
procedure TfrmTest4ChEEPROM.CreateGui(nChCount : Integer);
var
  i, nItemHeight, nItemWidth : Integer;
  nFontSize : Integer;
  sTemp : string;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.CreateGui'); //IMSI
...
function TfrmTest4ChEEPROM.DisplayPatList(sPatGrpName: string): TPatterGroup;
var
  CurPatGrp   : TPatterGroup;
  i           : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.DisplayPatList'); //IMSI
...
procedure TfrmTest4ChEEPROM.DisplayPGStatus(nPgNo, nType: Integer; sMsg: string);
var
  nCh : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.DisplayPGStatus'); //IMSI
  nCh := nPgNo mod 4;
  try
    case nType of
...
procedure TfrmTest4ChEEPROM.DisplayPwrData(nPgNo: Integer; PwrData: ReadVoltCurr);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.DisplayPwrData'); //IMSI
...
procedure TfrmTest4ChEEPROM.DisplaySysInfo;
var
  i, nPgNo: Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.DisplaySysInfo'); //IMSI
...
procedure TfrmTest4ChEEPROM.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.FormCloseQuery'); //IMSI
...
procedure TfrmTest4ChEEPROM.FormCreate(Sender: TObject);
var
  i: Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.FormCreate'); //IMSI
  ...
  EepromPlcCtl.OnPlcBcrRead := getEepromPlcBcrData;
end;

procedure TfrmTest4ChEEPROM.FormDestroy(Sender: TObject);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.FormDestroy'); //IMSI
...
procedure TfrmTest4ChEEPROM.getBcrData(sScanData: string);
var
  i, nLastCh: Integer;
  bTemp : Boolean;
  sDebug : string;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.getBcrData'); //IMSI
  sDebug := 'BCR Checking.';
  bTemp := True;
  nLastCh := -1;
  for i := DefCommon.CH1 to Pred(Common.SystemInfo.ChCountUsed) do begin
    if (pnlSerials[i].Caption = '') and (chkChannelUse[i].Checked) and (Pg[i].Status = pgConnect) then begin
      pnlSerials[i].Caption := sScanData;
      if sScanData.Length <> 20 then begin
        pnlPGStatuses[i].Caption := 'BCR Length NG';
        pnlPGStatuses[i].Color := clRed;
        pnlPGStatuses[i].Font.Color := clBlack;
        Exit;
      end;
      Logic[Self.Tag * 4 + i].m_Inspect.SerialNo := sScanData;
  ...
  if DongaGmes is TGmes then begin
    ...
    if not bTemp then begin
      if nLastCh > -1 then begin
        DongaGmes.SendHostPchk(sScanData,nLastCh);
        Exit;
      end;
    end;
  end;
...
procedure TfrmTest4ChEEPROM.getEepromPlcBcrData(nIndexPos:Smallint; naBcrLen: array of Integer; saBcrData: array of string);
var
  i : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.getEepromPlcBcrData'); //IMSI
  for i := DefCommon.CH1 to DefCommon.CH4 do begin
    //IMSI-ORG if (pnlSerials[i].Caption = '') and (chkChannelUse[i].Checked) and (Pg[i].Status = pgConnect) then begin
    if (pnlSerials[i].Caption = '') and (chkChannelUse[i].Checked) then begin
      pnlSerials[i].Caption := saBcrData[i];
      Logic[Self.Tag * 4 + i].m_Inspect.SerialNo := saBcrData[i];
      Logic[Self.Tag * 4 + i].m_Inspect.IsScanned := True;
    end;

    pasScr[i].SetBCRData;
    if DongaGmes is TGmes then begin
      //TBD DongaGmes.SendHostZset(Logic[i].m_Inspect.SerialNo, Logic[i].m_Inspect.JigId);
      // Main에서 ZSET 상위보고 정상 수신되면 스크립트 실행
//    PasScr[i].RunSeq(DefScript.SEQ_KEY_SCAN);
    end
    else begin
      // 바로 진행시킬건지?
//    PasScr[i].RunSeq(DefScript.SEQ_KEY_SCAN);
...
procedure TfrmTest4ChEEPROM.gridPatternListClick(Sender: TObject);
var
  nIdx, nPatType : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.gridPatternListClick'); //IMSI
...
procedure TfrmTest4ChEEPROM.OnTotalTimer(Sender: TObject);
var
  nSec, nMin : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.OnTotalTimer'); //IMSI
  Inc(m_nTotalTact);
...
procedure TfrmTest4ChEEPROM.RevSwDataJig(sGetData: String);
var
  nPos: Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.RevSwDataJig'); //IMSI
...
procedure TfrmTest4ChEEPROM.btnAutoClick(Sender: TObject);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.btnAutoClick'); //IMSI
//  JigLogic[Self.Tag].StartIspd_A_Auto;
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_1);
end;

procedure TfrmTest4ChEEPROM.btnCh2Click(Sender: TObject);
//var
//  i : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.btnCh2Click'); //IMSI
//  i := Self.Tag*4 + 1;
//  Logic[i].StartEachCh(i);
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_8);
end;

procedure TfrmTest4ChEEPROM.btnCh4Click(Sender: TObject);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.btnCh4Click'); //IMSI
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_7);
end;

procedure TfrmTest4ChEEPROM.RzBitBtn2Click(Sender: TObject);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.RzBitBtn2Click'); //IMSI
//  JigLogic[Self.Tag].NextIspd(False);
  JigLogic[Self.Tag].StartIspd_A(DefScript.SEQ_KEY_4);
end;

procedure TfrmTest4ChEEPROM.RzBitBtn7Click(Sender: TObject);
//var
//  i : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.RzBitBtn7Click'); //IMSI
...
procedure TfrmTest4ChEEPROM.RzBitBtn8Click(Sender: TObject);
//var
//  i : Integer;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.RzBitBtn8Click'); //IMSI
//  for i := Self.Tag*4 to Pred(Self.Tag*4 + 4) do begin
...
procedure TfrmTest4ChEEPROM.SetBcrData;
begin
  DongaHandBcr.OnRevBcrData := getBcrData;
end;

procedure TfrmTest4ChEEPROM.SetEepromPlcBcrData;
begin
  EepromPlcCtl.OnPlcBcrRead := getEepromPlcBcrData;
end;

procedure TfrmTest4ChEEPROM.SetConfig;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.SetConfig'); //IMSI
//  DisplaySeq;
  DisplaySysInfo;
...
procedure TfrmTest4ChEEPROM.SetHandleAgain(hMain: HWND);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.SetHandleAgain'); //IMSI
  JigLogic[Self.Tag].SetHandleAgain(hMain, Self.Handle);
end;

//procedure TfrmTest4ChEEPROM.SetLanguage(nIdx: Integer);
//var
//  i : Integer;
//begin
//  btnStartTest.Caption := 'bắt đầu (START)';
...
//procedure TfrmTest4ChEEPROM.ShowAgingTime(nTime: Integer);
//var
...
procedure TfrmTest4ChEEPROM.ShowGui(hMain : HWND);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.ShowGui'); //IMSI
...
procedure TfrmTest4ChEEPROM.tmrDisplayOffTimer(Sender: TObject);
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.tmrDisplayOffTimer'); //IMSI
  tmrDisplayOff.Enabled := False;
  pnlErrAlram.Visible := False;
end;

procedure TfrmTest4ChEEPROM.UpdatePtList;
var
  nCh : Integer;
  PatGrp : TPatterGroup;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.UpdatePtList'); //IMSI
...
procedure TfrmTest4ChEEPROM.WMCopyData(var Msg: TMessage);
var
  nType, nMode, nCh, i, nTemp, nPatType : Integer;
  bTemp : Boolean;
  sMsg, sDebug, sTemp : string;
begin
  Common.MLog(DefCommon.MAX_SYSTEM_LOG,'TfrmTest4ChEEPROM.WMCopyData'); //IMSI
