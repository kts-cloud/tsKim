...
//{$ENDIF}
  ...
  COMMPLC_MODE_LOAD_1 = 1;  //Glass Data Request
  COMMPLC_MODE_LOAD_2 = 2;  //Glass Data Report
  COMMPLC_MODE_LOAD_3 = 3;  // Load Request
  COMMPLC_MODE_LOAD_4 = 4;  // Load Enable
  COMMPLC_MODE_LOAD_5 = 5;  // Robot Busy
  COMMPLC_MODE_LOAD_6 = 6;  // Load Complete
  COMMPLC_MODE_LOAD_7 = 7;  // Load Complete Confirm
  COMMPLC_MODE_LOAD_11 = 11;// Load EQP Normal Status
  COMMPLC_MODE_LOAD_12 = 12;// Load ROBOT Normal Status

  COMMPLC_MODE_UNLOAD_1 = 21;    // Glass Data Report
  COMMPLC_MODE_UNLOAD_2 = 22;    // UnLoad Request
  COMMPLC_MODE_UNLOAD_3 = 23;    // UnLoad Enable
  COMMPLC_MODE_UNLOAD_4 = 24;    // Robot Busy
  COMMPLC_MODE_UNLOAD_5 = 25;    // UnLoad Complete
  COMMPLC_MODE_UNLOAD_6 = 26;    // UnLoad Complete Confirm
  COMMPLC_MODE_UNLOAD_11 = 31;   // UnLoad EQP Normal Status
  COMMPLC_MODE_UNLOAD_12 = 32;   // UnLoad ROBOT Normal Status
...
//    procedure SendMessageTest(nMsgMode, nCh, nParam, nParam2: Integer; sMsg: String; pData: Pointer=nil);
    ...
    procedure Process_ROBOT_Vacuum(nCh,nValue: Integer);

    procedure Process_Door_Open_Warning(nValue: Integer);
    procedure Process_Door_Open_Info(nValue: Integer);
...
//    function ReadDeviceBlock(const szDevice: String; lSize: Integer; out lplData: Integer): integer;
  ...
  /// <summary> PLC에서 문자열 데이터 읽기 </summary>
    ...
    function EQP_Door_Open_Warning: Boolean;
    procedure EQP_Door_Open_Info(nValue: Integer);
...
begin
  ...
  if (Common.SystemInfo.OCType = DefCommon.OCType) then
    m_nEQPDataSize := 16
  else if (Common.SystemInfo.OCType = DefCommon.PreOCType) and (Common.PLCInfo.InlineGIB) then
    m_nEQPDataSize := 24
  else  m_nEQPDataSize := 22;
  ...
  SetLength(PollingEQP, m_nEQPDataSize);
  SetLength(PollingEQPPre, m_nEQPDataSize);
...
//  end
//  );
  if (Common.SystemInfo.OCType = DefCommon.PreOCType) and (Common.PLCInfo.InlineGIB)  then
    WriteDeviceBlock('W' + IntToHex(StartAddr_EQP_W+$10*$20+$0 + (nCh*$40) , 3), 64, naGlassData[0])
  else
  ...
  if bExist then begin
    if (Common.PLCInfo.InlineGIB)  then begin
      if (Common.SystemInfo.OCType = DefCommon.OCType) then  begin
        ...
        Sleep(10);
        WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$0C+(nCh), 3), 1) //Position Glass Exist
      end;
    end
    else begin
      if Common.SystemInfo.OCType = DefCommon.OCType  then
    ...
    if (Common.PLCInfo.InlineGIB) then begin
      if (Common.SystemInfo.OCType = DefCommon.OCType) then begin
        ...
        Sleep(10);
        WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$0C+(nCh), 3), 0) //Position Glass Exist
      end;
    end
    else begin
      if Common.SystemInfo.OCType = DefCommon.OCType  then
...
//    sEcsBitAddr :=  'B'+IntToHex(StartAddr_ECS+ $100+nIndex, 3)
    sECSBitAddr := 'B'+IntToHex(StartAddr_ECS+ $100 + (EQP_ID + 10) mod 16 , 3);
  end
  else begin
    sEcsBitAddr := 'B'+IntToHex(StartAddr_ECS+ $100+nIndex, 3);
  end;
...
procedure TCommPLCThread.EQP_Door_Open_Info(nValue: Integer);
begin
  if not Connected then Exit;
  AddLog('EQP_Door_Open_Info ' + IntToStr(nValue));
  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$05+$2 , 3), nValue); //EQP_Door_Open_Info

end;

function TCommPLCThread.EQP_Door_Open_Warning: Boolean;
var
nRet : integer;
begin
  Result := True;
  AddLog('Process_EQP_Door_Open_Warning ' + IntToStr(1));

  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$05+$0 , 3), 1); //Door Open Warning ON

  nRet:= WaitSignal('B' + IntToHex(StartAddr_ROBOT_DOOR_BIT+$10*$00+$2, 3), 1, 3000); //Door Open Warning Confirm 대기
  if nRet <> 0 then begin
    AddLog('Door Open Warning Confirm ON Timeout');
    //오류
    WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$05+$0 , 3), 0);
    Exit(false);
  end;

  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$05+$0 , 3), 0); //Door Open Warning OFF

  nRet:= WaitSignal('B' + IntToHex(StartAddr_ROBOT_DOOR_BIT+$10*$00+$2, 3), 0, 3000); //Door Open Warning Confirm 대기
  if nRet <> 0 then begin
...
begin
  ...
  if (Common.PLCInfo.InlineGIB) then begin
    sECSBitAddr := 'B'+IntToHex(StartAddr_ECS+ $10 * $20 + (EQP_ID + 10) mod 16, 3);
  end
  else begin
...
begin
  ...
  if (Common.PLCInfo.InlineGIB) then begin
    if (Common.SystemInfo.OCType = DefCommon.OCType)  then  begin
      ...
      Sleep(100);
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10+$1 + (nCh*$20), 3), 1); //Load Glass Data Request
      RequestState_Unload[nCh]:= 1;
      RequestState_Load[nCh]:= 1;
...
begin
  ...
  if (Common.PLCInfo.InlineGIB) then begin
    if (Common.SystemInfo.OCType = DefCommon.OCType) then begin
      ...
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$4 + (nCh*$20), 3), 1); //Unload Normal Status
      Common.StatusInfo.LoadUnloadFlowData[nCh][COMMPLC_MODE_UNLOAD_11] := 1;
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$0 + (nCh*$20), 3), 0); //Unload Enable
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$1 + (nCh*$20), 3), 0); //Glass Data Report
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$5 + (nCh*$20), 3), 0); //Unload Request
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$6 + (nCh*$20), 3), 0); //Unload Complete Confrim
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10+$0 + (nCh*$20), 3), 0); //Load Enable Off
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10+$5 + (nCh*$20), 3), 0); //Load Request Off
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10+$6 + (nCh*$20), 3), 0); //Load Complte Confirm Off
    ...
    if Common.SystemInfo.OCType = DefCommon.PreOCType then begin
      if Common.PLCInfo.InlineGIB then begin
        for I := DefCommon.CH1 to DefCommon.MAX_CH do begin
          WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10 + (i * $20) + $E, 3), ControlDio.IsPreOCInterlockPROBE_CH(i), False);
          WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10 + (i * $20) + $F, 3), ControlDio.IsPreOCInterlockSHUTTER(i div 2), False);
        end;
        if Common.TestModelInfoFLOW.ModelTypeName = 'X2146' then begin
          WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$01 + $4, 3), 1, False);
          WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$01 + $5, 3), 0, False);
        end
        else begin
...
//    Result:= 1; //읽기 실패
  ...
  if Common.SystemInfo.OCType = DefCommon.OCType then begin
    nRet := ReadDeviceBlock('B' + IntToHex(StartAddr_ROBOT_DOOR_BIT, 3), COMMPLC_CV_DATASIZE, PollingCV[0],nRet,False);
...
//  Sleep(50);
  ...
  m_ResultEQP :=  ReadDeviceBlock('B' + IntToHex(StartAddr_EQP+$10*$00+$0, 3), m_nEQPDataSize, PollingEQP[0],m_ResultEQP, False);
...
function IntegerToBoolean(value: Integer): Boolean;
begin
  Result := value <> 0;
end;
  ...
  for i := 0 to Pred(COMMPLC_CV_DATASIZE) do begin
    ...
    for k := 0 to 15 do begin
      ...
      if nValue <> Get_Bit(PollingCVPre[i], k) then begin
        nIndex:= i*16 + k;
        AddLog(format('<< ChangedDevice CV %s: %d', ['B' + IntToHex(StartAddr_ROBOT_DOOR_BIT+nIndex, 3), nValue]), True);
        if (Common.PLCInfo.InlineGIB) then begin
          case nIndex of
            $01 : begin    //Door Open Warning
              if nValue <> 0 then Process_Door_Open_Warning(1)
              else  Process_Door_Open_Warning(0);
            end;
            ...
            $03 : begin
              if nValue <> 0 then Process_Door_Open_Info(1)
              else  Process_Door_Open_Info(0);
            end;
...
procedure TCommPLCThread.Process_Door_Open_Warning(nValue: Integer);
begin
  AddLog('Process_Door_Open_Warning ' + IntToStr(nValue));
  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$05+$1 , 3), nValue); //Door Open Warning Confirm ON/OFF
end;

procedure TCommPLCThread.Process_Door_Open_Info(nValue: Integer);
begin
  AddLog('Process_Door_Open_Info ' + IntToStr(nValue));
  SendMessageMain(COMMPLC_MODE_EVENT_ROBOT, 0, COMMPLC_PARAM_DOOR_OPENED, nValue, 'Robot_DoorOpened', nil);
end;
...
//      Sleep(100);
      ...
      format('%s (%s)', [GlassData[nCh].CarrierID, GlassData[nCh].GlassID]), nil);
    end
    else begin
      ReadDeviceBlock('W' + IntToHex(StartAddr_ROBOT_W+$10*$0+$0+(nCh *$40), 3), 64, naGlassData[0],nReturnCode); //Load #1 Glass Data
      ConvertBlockToGlassData(naGlassData[0], GlassData[nCh]);
...
procedure TCommPLCThread.Process_ROBOT_Vacuum(nCh, nValue: Integer);
begin
  AddLog(format('Process_ROBOT_LoadVacuum Ch=%d, Value=%d', [nCh, nValue]));

  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$10+$A + (nCh*$20), 3), nValue); //ROBOT_LoadVacuum
  WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$A + (nCh*$20), 3), nValue); //ROBOT_LoadVacuum
end;
...
begin
  ...
  Common.StatusInfo.LoadUnloadFlowData[nCh][COMMPLC_MODE_LOAD_5] := 1;
...
//    if (CSharpDll.MainOC_GetOCFlowIsAlive(2*nCh) = 1)  then Exit;
    if (Common.SystemInfo.OCType = DefCommon.OCType) then begin
      ...
      Common.StatusInfo.LoadUnloadFlowData[nCh][COMMPLC_MODE_UNLOAD_6] := 1;
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$6 + (nCh*$20), 3), 1); //Unload Complete Confrim
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$5 + (nCh*$20), 3), 0); //Unload Request Off
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$1 + (nCh*$20), 3), 0); //Glass Data Request Off
    end;
  end
  else begin
    if (CSharpDll.MainOC_GetOCFlowIsAlive(2*nCh) = 1) or (CSharpDll.MainOC_GetOCFlowIsAlive(2*nCh +1) = 1) then Exit;
...
begin
  ...
  if (Common.PLCInfo.InlineGIB)  then  begin
    if (Common.SystemInfo.OCType = DefCommon.OCType)  then
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$09+$6 + (nCh*$20), 3), 0) //Unload Complete Confrim Off
    else WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$11+$6 + (nCh*$20), 3), 0) //Unload Complete Confrim Off
  end
  else begin
    if Common.SystemInfo.OCType = DefCommon.OCType then
      WriteDevice('B' + IntToHex(StartAddr_EQP+$10*$0D+$6 + (nCh*$20), 3), 0) //Unload Complete Confrim Off
...
begin
  ...
  else if nBitCount = 2 then begin     //Pre OC GIB
    if nStation = 1 then
      GlassData.GlassProcessingStatus[nSeq] := 1;
    if nStation = 2 then
      GlassData.GlassProcessingStatus[nSeq] := 4;
  end
